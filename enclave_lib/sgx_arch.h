...
typedef struct {
    uint8_t user : 1;
    uint8_t sse_ignore : 1;
    uint8_t reserved : 2;
    uint8_t target_ssa : 4;
} sgx_switch_flag_t;
static_assert(sizeof(sgx_switch_flag_t) == 1, "incorrect struct size");
...
static_assert(sizeof(sgx_eraise_info_t) == 16, "invalid size");
