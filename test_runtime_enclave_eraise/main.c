                        ...
                        struct enclave_run *run, uint64_t r8, uint64_t r9)
{
    ...
    if (run->exit_reason == EXIT_EEXIT)
    {
        ...
        uint64_t * exception_tsc_addr = (uint64_t *)((uintptr_t)eexit_tsc_addr + 8);
        uint64_t * eraise_tsc_addr = (uint64_t *)((uintptr_t)exception_tsc_addr + 8);
        printf("exception_tsc: %lu, aex_tsc: %lu, eraise_tsc: %lu\n", *exception_tsc_addr, *aex_tsc_addr, *eraise_tsc_addr);
#endif  
        return 0;
        case EEXIT_EXIT:
